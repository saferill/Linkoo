...
async fn handle_request_inner(mut req: Request<Incoming>) -> Result<Response<BoxedBody>, AppError> {
    ...
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/v1/download/initiate") | (&Method::POST, "/api/linko/v2/prepare-download") => {
            web::prepare_download(req, state, client_info).await
        }
        (&Method::GET, "/v1/download/stream") | (&Method::GET, "/api/linko/v2/download") => {
            web::download(req, state, client_info).await
        }
        (&Method::POST, "/v1/node/handshake") | (&Method::POST, "/api/linko/v2/register") => {
            if !v2_enabled {
        ...
        (&Method::GET, "/v1/node/status") | (&Method::GET, "/api/linko/v1/info") | (&Method::GET, "/api/linko/v2/info") => {
            if !v2_enabled {
                return Err(AppError::Status(StatusCode::NOT_FOUND));
            }
        ...
        (&Method::POST, "/v1/transfer/initiate") | (&Method::POST, "/api/linko/v2/prepare-upload") => {
            if !v2_enabled {
                return Err(AppError::Status(StatusCode::NOT_FOUND));
            }
        ...
        (&Method::POST, "/v1/transfer/stream") | (&Method::POST, "/api/linko/v2/upload") => {
            if !v2_enabled {
                return Err(AppError::Status(StatusCode::NOT_FOUND));
            }
        ...
        (&Method::POST, "/v1/transfer/abort") | (&Method::POST, "/api/linko/v2/cancel") => {
            if !v2_enabled {
                return Err(AppError::Status(StatusCode::NOT_FOUND));
            }
        ...
        (&Method::POST, "/v1/system/bring-to-front") | (&Method::POST, "/api/linko/v2/show") => internal::show(req, state).await,
        (&Method::POST, "/v1/security/nonce") | (&Method::POST, "/api/linko/v3/nonce") => {
            Ok(v3::nonce_exchange(req.into_body(), state, client_info)
                .await?
                .into_response())
        }
        (&Method::POST, "/v1/security/register") | (&Method::POST, "/api/linko/v3/register") => {
