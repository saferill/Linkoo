...
//! `fe80--1s3.scoped.linko.internal`, and [`decode`], called by the custom
...
const SUFFIX: &str = ".scoped.linko.internal";
...
mod tests {
    ...
    fn test_encode_scoped_host() {
        assert_eq!(
            ...
            Some("fe80--1s3.scoped.linko.internal")
        );
        assert_eq!(
            encode("fe80::abcd:ef12:3456:789a%4294967295").as_deref(),
            Some("fe80--abcd-ef12-3456-789as4294967295.scoped.linko.internal")
        );
    }
    ...
    fn test_decode_rejects_other_names() {
        ...
        assert_eq!(decode("fe80--1.scoped.linko.internal"), None); // no scope
        assert_eq!(decode("fe80--1sx.scoped.linko.internal"), None); // bad scope
        assert_eq!(decode("s3.scoped.linko.internal"), None); // no address
    }
}
