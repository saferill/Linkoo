//! UDP multicast discovery for the Linko protocol (v2.2).
//!
...
/// The multicast group used by Linko.
///
...
pub const DEFAULT_MULTICAST_GROUP: Ipv4Addr = Ipv4Addr::new(224, 0, 0, 199);

/// The IPv6 multicast group used by Linko, a transient (`ff1x::`) group
/// with link-local scope.
///
/// IPv6 discovery is a Linko extension on top of protocol v2.2, so IPv4
/// remains the baseline and IPv6 is announced in parallel.
pub const DEFAULT_MULTICAST_GROUP_V6: Ipv6Addr =
    Ipv6Addr::new(0xff12, 0, 0, 0, 0, 0, 0xa8b2, 0x7710);

/// The default multicast port, identical to the default HTTP server port.
pub const DEFAULT_PORT: u16 = 48855;
...
impl MulticastHandle {
    /// Announces this device to the network, which makes every other Linko
