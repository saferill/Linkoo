...
use linko::crypto::hash::{sha256_file_content, sha256_hex, HashError};
use linko::model::transfer::FileContent;
...
async fn hash_file_from_path() {
    let path = std::env::temp_dir().join(format!("linko-hash-{}", uuid::Uuid::new_v4()));
