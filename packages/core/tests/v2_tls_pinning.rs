...
use linko::http::server::web::{WebConfig, WebMode};
use linko::http::server::{start_with_port, ServerConfigV2, TlsConfig};
use linko::http::state::ClientInfo;
use linko::model::discovery::ProtocolType;
use linko::model::transfer::FileDto;
...
/// A generated self-signed certificate, as Linko peers use them.
struct Identity {
    cert: String,
    private_key: String,
...
fn generate_identity() -> Identity {
    let cert = linko::crypto::cert::generate_self_signed().unwrap();
...
async fn test_transfer_with_matching_fingerprint() {
    ...
    let content = b"hello linko".repeat(100);
...
async fn test_client_without_cert_allowed_in_web_mode() {
    ...
    let browser = linko::reqwest::Client::builder()
        .use_rustls_tls()
    ...
    // A Linko peer still authenticates with its certificate.
    let client = client(&sender, Some(&server_identity.fingerprint));
    let response = client
        .register(
            ProtocolType::Https,
