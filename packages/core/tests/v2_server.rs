...
use linko::crypto::hash::sha256_hex;
use linko::http::client::{ClientError, LsHttpClientV2};
use linko::http::dto_v2::{PrepareUploadRequestDtoV2, RegisterDtoV2};
use linko::http::server::common::save::FileUploadTarget;
use linko::http::server::v2::{PrepareUploadDecisionV2, ServerEventV2, SessionEndReasonV2};
use linko::http::server::web::WebConfig;
...
use linko::model::transfer::{FileDto, FileMetadata};
...
async fn upload_bytes(
    ...
    let body =
        linko::reqwest::Body::wrap_stream(ReceiverStream::new(rx).map(move |chunk: Bytes| {
...
async fn test_info_on_legacy_v1_route() {
    ...
    let body: serde_json::Value = reqwest::get(format!(
        "http://127.0.0.1:{}/api/linko/v1/info",
        server.port
    ))
    .await
    .unwrap()
...
async fn test_upload_applies_file_timestamps() {
    let save_dir = std::env::temp_dir().join(format!("linko-test-{}", uuid::Uuid::new_v4()));
...
async fn test_upload_missing_parameters() {
    ...
    let response = linko::reqwest::Client::new()
        .post(format!(
            "http://127.0.0.1:{}/api/linko/v2/upload?sessionId=abc",
            server.port
        ))
        .body("data")
        .send()
...
async fn test_prepare_upload_aborted_by_sender_disconnect() {
    ...
    let request = format!(
        "POST /api/linko/v2/prepare-upload HTTP/1.1\r\nHost: 127.0.0.1:{port}\r\nContent-Type: application/json\r\nContent-Length: {}\r\n\r\n{body}",
        body.len()
    );
...
async fn test_prepare_upload_cancelled_by_session_less_cancel() {
    ...
    let cancel_url = format!("http://127.0.0.1:{port}/api/linko/v2/cancel");
    let cancel_client = linko::reqwest::Client::new();
...
/// with mutual certificates - the transport every real Linko transfer
...
async fn test_prepare_upload_aborted_by_sender_disconnect_tls() {
    ...
    let server_cert = rcgen::CertificateParams::new(vec!["Linko User".to_string()])
        .unwrap()
        .self_signed(&server_key)
        .unwrap();
    let sender_key = rcgen::KeyPair::generate().unwrap();
    let sender_cert = rcgen::CertificateParams::new(vec!["Linko User".to_string()])
        .unwrap()
    ...
    let handle = start_with_port(
        0,
        Some(linko::http::server::TlsConfig {
