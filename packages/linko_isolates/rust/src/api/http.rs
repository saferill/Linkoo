...
pub use linko::http::client::{ClientError, LsHttpClientVersion};
pub use linko::http::dto::{
...
use linko::reqwest;
use linko::util::error::ErrorChain;

pub struct RsHttpClient {
    inner: linko::http::client::LsHttpClient,
}
...
pub fn create_client(
    ...
    let inner = linko::http::client::LsHttpClient::new(
        &private_key,
        &cert,
        version,
...
fn resolve_file_content(
    ...
    path: Option<String>,
    file_descriptor: Option<i32>,
) -> Result<linko::model::transfer::FileContent, RsHttpClientError> {
    match (binary, path, file_descriptor) {
        (Some(binary), None, None) => Ok(linko::model::transfer::FileContent::Stream(
            binary.receiver,
        )),
        (None, Some(path), None) => Ok(linko::model::transfer::FileContent::Path(path.into())),
        (None, None, Some(file_descriptor)) => {
            #[cfg(target_os = "android")]
            {
                Ok(linko::model::transfer::FileContent::Fd(file_descriptor))
            }
