...
impl ProposingClientInfo {
    fn sign(&self, signing_key: &SigningTokenKey) -> anyhow::Result<ClientInfoWithoutId> {
        Ok(ClientInfoWithoutId {
            ...
            token: linko::crypto::token::generate_token_timestamp(&signing_key)?,
        })
    }
}
...
pub async fn connect(
    ...
    let Ok(signing_key) = linko::crypto::token::parse_private_key(&private_key) else {
...
impl LsSignalingConnection {
    ...
    pub async fn send_offer(
        ...
        let signing_key = linko::crypto::token::parse_private_key(private_key)?;
        let expecting_public_key = match expecting_public_key {
            Some(key) => Some(linko::crypto::token::parse_public_key(
                &key.public_key,
                &key.kind,
            )?),
            None => None,
        ...
        tokio::spawn(async move {
            let result = linko::webrtc::webrtc::send_offer(
                &managed_connection,
                stun_servers,
                target,
                signing_key,
    ...
    pub async fn accept_offer(
        ...
        tokio::spawn(async move {
            let result = linko::webrtc::webrtc::accept_offer(
                &managed_connection,
                stun_servers,
                &offer,
                signing_key,
