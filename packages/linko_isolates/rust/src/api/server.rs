...
pub use linko::http::dto_v2::RegisterDtoV2;
use linko::http::server::ServerConfigV2;
pub use linko::http::server::TlsConfig;
use linko::http::server::common::save::FileUploadTarget;
use linko::http::server::internal::{InternalConfig, InternalEvent};
pub use linko::http::server::v2::SessionEndReasonV2;
use linko::http::server::v2::{PrepareUploadDecisionV2, ServerEventV2};
use linko::http::server::web::{
    WebConfig, WebMode as CoreWebMode, WebDownloadConfig, WebDownloadEvent,
};
pub use linko::http::server::web::{WebI18n, WebPages};
use linko::http::state::ClientInfo;
use linko::model::discovery::DeviceType;
...
pub enum RsServerEvent {
    /// A device registered itself via `POST /api/linko/v2/register`.
    ///
    ...
    /// A sender requests to upload files via `POST /api/linko/v2/prepare-upload`.
    PrepareUpload {
    ...
    /// An accepted file is being uploaded via `POST /api/linko/v2/upload`.
    FileUpload {
        session_id: String,
        file_id: String,
        file: FileDto,
    ...
    /// `POST /api/linko/v2/cancel` was received for a session this server
    ...
    /// A web client requests to download the shared files via `POST /api/linko/v2/prepare-download`.
    ///
    ...
    /// A web client downloads an offered file via `GET /api/linko/v2/download`.
    ///
...
struct ServerInstance {
    handle: linko::http::server::ServerHandle,
...
pub async fn start_server(
    ...
    let (web_mode, web_event_rx) = match web.mode {
        WebMode::Disabled => (CoreWebMode::Disabled, None),
        WebMode::Download { files, pin } => {
            let (web_event_tx, web_event_rx) = mpsc::channel::<WebDownloadEvent>(16);
            let mode = CoreWebMode::Download(WebDownloadConfig {
                files,
                pin,
                event_tx: web_event_tx,
            });
            (mode, Some(web_event_rx))
        }
        WebMode::Upload => (CoreWebMode::Upload, None),
    };
    ...
    let handle = linko::http::server::start_with_port(
        port,
        tls,
        ClientInfo {
            alias,
