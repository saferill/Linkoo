...
pub fn verify_cert(cert: String, public_key: String) -> anyhow::Result<()> {
    linko::crypto::cert::verify_cert_from_pem(cert, Some(&public_key))
}

pub fn generate_key_pair() -> anyhow::Result<KeyPair> {
    let signing_key = linko::crypto::token::generate_key();
    let private_key = linko::crypto::token::export_private_key(&signing_key)?;
    let public_key = linko::crypto::token::export_public_key(&signing_key)?;
...
pub fn generate_security_context() -> anyhow::Result<SecurityContext> {
    let cert = linko::crypto::cert::generate_self_signed()?;
...
pub async fn hash_file(
    ...
    let result = async {
        let content = match (path, file_descriptor, bytes) {
            (Some(path), None, None) => linko::model::transfer::FileContent::Path(path.into()),
            (None, Some(file_descriptor), None) => {
                #[cfg(target_os = "android")]
                {
                    linko::model::transfer::FileContent::Fd(file_descriptor)
                }
            ...
            (None, None, Some(bytes)) => {
                let hash = linko::crypto::hash::sha256_hex(&bytes);
        ...
        let hash =
            linko::crypto::hash::sha256_file_content(content, &cancel_token.inner, progress)
                .await?;
        let _ = sink.add(RsHashFileEvent::Done { hash });
        Ok(())
    }
