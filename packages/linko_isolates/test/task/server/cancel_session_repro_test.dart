...
// build it with `cargo build -p rust_lib_linko_app` from the repo root.
...
void main() {
  test('new session works after receiver-side cancel during transfer', () async {
    final dylib = File('${Directory.current.path}/../../target/debug/librust_lib_linko_app.dylib');
    if (!dylib.existsSync()) {
    ...
    Future<(int, String)> prepareUpload(List<String> fileIds, {required int size}) async {
      final req = await client.postUrl(Uri.parse('http://127.0.0.1:$_port/api/linko/v2/prepare-upload'));
    ...
    Future<int> upload(String sessionId, String fileId, String token, int size, {bool abortable = false}) async {
      final req = await client.postUrl(
        Uri.parse('http://127.0.0.1:$_port/api/linko/v2/upload?sessionId=$sessionId&fileId=$fileId&token=$token'),
      );
