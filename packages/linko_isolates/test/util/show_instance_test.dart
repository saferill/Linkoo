// Regression test for https://github.com/linko/linko/issues/3165
//
...
import 'package:linko_isolates/rust/api/crypto.dart' hide SecurityContext;
...
File _findDylib() {
  ...
  return [
    File('$base/librust_lib_linko_app.dylib'),
    File('$base/librust_lib_linko_app.so'),
    File('$base/rust_lib_linko_app.dll'),
  ].firstWhere((f) => f.existsSync(), orElse: () => File('$base/librust_lib_linko_app.dylib'));
}
...
void main() {
  ...
  Future<(HttpServer, _Received)> startStub({int statusCode = 200}) async {
    ...
    server.listen((request) async {
      ...
      request.response.statusCode = switch (received) {
        ...
        _Received(path: != '/api/linko/v2/show') => HttpStatus.notFound,
  ...
  test('hands the arguments to the running instance over TLS', () async {
    if (!_findDylib().existsSync()) {
      markTestSkipped('Rust dylib not built (cargo build -p rust_lib_linko_app)');
      return;
    }
    ...
    expect(received.path, '/api/linko/v2/show');
