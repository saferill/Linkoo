...
/// Linko CLI
#[derive(Parser)]
#[command(name = "linko-cli", version, about, after_help = HELP_SECTIONS)]
pub struct Args {
    /// Device name shown to other devices [default: config.toml, else the hostname]
    #[arg(long, env = "LINKO_ALIAS")]
    pub alias: Option<String>,

    /// Port of the HTTP server [default: config.toml, else 48855]
    #[arg(long, env = "LINKO_PORT")]
    pub port: Option<u16>,

    /// Directory where received files are saved [default: config.toml, else the Downloads folder]
    #[arg(long, env = "LINKO_DESTINATION")]
...
const HELP_SECTIONS: &str = "Events:\n  \
                             ...
                             XDG_CONFIG_HOME, LINKO_ALIAS, LINKO_PORT, LINKO_DESTINATION";
...
mod tests {
    ...
    fn accepts_mixed_send_paths() {
        let args = Args::try_parse_from(["linko-cli", "send", "one.txt", "two.txt", "backup"])
            .unwrap();
    ...
    fn accepts_a_destination_alias() {
        let args =
            Args::try_parse_from(["linko-cli", "send", "--to", "Cute Tomato", "one.txt"])
                .unwrap();
    ...
    fn accepts_a_destination_ip() {
        let args =
            Args::try_parse_from(["linko-cli", "send", "--to", "192.168.27.26", "one.txt"])
                .unwrap();
    ...
    fn requires_at_least_one_send_path() {
        assert!(Args::try_parse_from(["linko-cli", "send"]).is_err());
    }

    #[test]
    fn accepts_interactive_mode_without_a_command() {
        let args = Args::try_parse_from(["linko-cli"]).unwrap();
